    ...
    def predict_example(self, tree_path, weights_path):
        ...
        with tf.Session() as sess:
            ...
            root_prediction, logits = sess.run(
                [self.root_prediction, self.scores], feed_dict=feed_dict
            )
        ...
        with tf.Session() as sess:
            ...
            for batch in range(num_batches):
                ...
                loss_value, acc, _ = sess.run(
                    [self.full_loss, self.root_acc, self.train_op], feed_dict=feed_dict
                )
                if verbose:
                    sys.stdout.write(
    ...
    def train(self, verbose=True, test=False):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            if (dev_acc == best_dev_acc) and (dev_loss < best_dev_loss):
                ...
                self.copy_weight_files(
                    self.config.model_name + ".loss", self.config.model_name + ".temp"
                )
        ...
        if test:
            _, test_loss, test_acc = self.predict(
                self.test_data, SAVE_DIR + "%s" % self.config.model_name, get_loss=True
            )
            print()
