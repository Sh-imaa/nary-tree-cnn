...
class TreeCNN:
    def __init__(self, config, train_data, dev_data, test_data=None):
        ...
        def combine_children(children_tensors):
            children_tensors = tf.reshape(
                children_tensors, [1, -1, self.config.embed_size]
            )
        ...
        loop_cond = lambda tensor_array, i: tf.less(
            i, tf.squeeze(tf.shape(self.is_leaf_placeholder))
        )
        ...
        self.root_logits = (
            tf.matmul(self.tensor_array.read(self.tensor_array.size() - 1), U) + bs
        )
    ...
    def build_feed_dict(self, node, label, train=True):
        ...
        feed_dict = {
            ...
            self.node_word_indices_placeholder: [
                self.vocab.encode(node.word) if node.word else -1 for node in nodes_list
            ],
            self.children_placeholder: [
    ...
    def run_epoch(self, new_model=False, verbose=True):
        ...
        with tf.Session() as sess:
            ...
            for step, tree in enumerate(self.train_data):
                ...
                loss_value, _ = sess.run(
                    [self.full_loss, self.train_op], feed_dict=feed_dict
                )
        ...
        train_preds, _ = self.predict(
            self.train_data, SAVE_DIR + "%s.temp" % self.config.model_name
        )
        val_preds, val_losses = self.predict(
            self.dev_data, SAVE_DIR + "%s.temp" % self.config.model_name, get_loss=True
        )
        ...
        for epoch in range(self.config.max_epochs):
            ...
            if val_loss < best_val_loss:
                self.copy_weight_files(
                    self.config.model_name, self.config.model_name + ".temp"
                )
    ...
    def copy_weight_files(self, new_path, old_path):
        shutil.copyfile(
            SAVE_DIR + "%s.index" % old_path, SAVE_DIR + "%s.index" % new_path
        )
        shutil.copyfile(
            SAVE_DIR + "%s.meta" % old_path, SAVE_DIR + "%s.meta" % new_path
        )
        shutil.copyfile(
